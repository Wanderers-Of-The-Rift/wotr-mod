...
public record RiftGenerationConfig(RiftLayout.Factory layout, RiftRoomGenerator.Factory roomGenerator,
    ...
    public static RiftGenerationConfig initialize(ItemStack itemStack, Long unused, RegistryAccess registries) {
        var preset = itemStack.get(WotrDataComponentType.RiftKeyData.GENERATOR_PRESET);
        if (preset == null) {
            preset = registries.holderOrThrow(DEFAULT_PRESET_KEY);
        }
        var config = preset.value();
        var layerEdits = itemStack.get(WotrDataComponentType.RiftKeyData.LAYOUT_LAYER_EDIT);
        ...
        var jigsawEdits = itemStack.get(WotrDataComponentType.RiftKeyData.JIGSAW_PROCESSORS_EDIT);
        if (jigsawEdits != null) {
        ...
        var postStepsEdits = itemStack.get(WotrDataComponentType.RiftKeyData.POST_STEPS_EDIT);
