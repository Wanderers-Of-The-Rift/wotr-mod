...
public class ChaoticRiftLayout implements RiftLayout{
    ...
    private ChaoticLayoutRegion getOrCreateRegion(ChunkPos position){
        ...
        return regions.computeIfAbsent(new Vector2i(regionX,regionZ),(unused)->new ChaoticLayoutRegion(new Vec3i(regionX*15-7,-ChaoticLayoutRegion.LAYERS/2,regionZ*15-7)));
    }
    ...
    public List<RiftSpace> getChunkSpaces(ChunkPos chunkPos, RandomState randomState) {
        ...
        var result = new ArrayList<RiftSpace>(ChaoticLayoutRegion.LAYERS);
        for (int i = 0; i < ChaoticLayoutRegion.LAYERS; i++) {
            result.add(region.getSpaceAt(new Vec3i(chunkPos.x,i-ChaoticLayoutRegion.LAYERS/2,chunkPos.z)));
        }
        return result;
    }
}
