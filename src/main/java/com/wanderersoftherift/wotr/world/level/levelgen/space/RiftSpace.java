...
public interface RiftSpace {
    ...
    static void placePerimeterInChunk(ChunkAccess chunk, RiftSpace space, int level, BlockState perimeterBlock) {
        var processedChunk = new RiftProcessedChunk(new Vec3i(chunk.getPos().x, level, chunk.getPos().z), null);
        placePerimeterInRiftChunk(processedChunk, space, perimeterBlock);
        processedChunk.placeInWorld(chunk, null);

    }

    static void placePerimeterInRiftChunk(RiftProcessedChunk chunk, RiftSpace space, BlockState perimeterBlock) {
        ...
        if (space == null || space instanceof VoidRiftSpace) {
            ...
            var corridors = space.corridors()
                    .stream()
                    .filter((corridor) -> corridor.position().getX() + space.origin().getX() == chunk.origin.getX()
                            && corridor.position().getZ() + space.origin().getZ() == chunk.origin.getZ()
                            && corridor.position().getY() + space.origin().getY() == level)
                    .toList();
